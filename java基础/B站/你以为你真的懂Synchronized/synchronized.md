...
![image-20200720111942758](C:\Users\pipi\AppData\Roaming\Typora\typora-user-images\image-20200720111942758.png)
...
>
> 重量级锁有等待队列，所有拿不到锁的进入等待队列，不需要消耗CPU资源. [用户态-> 内核态]
...
在STW的时候，所有的线程会停在`safe point`,此时会对未使用的`minoter`降级（只能被VMThread访问的线程）。其实，只被VMThread访问，降级也就没啥意义了。所以可以简单认为锁降级不存在！
